/**
 * Test script to analyze form submission data
...
async function analyzeFormData() {
  ...
  try {
    ...
    const formDoc = await gcpClient.getFormStructure(FORM_ID, true);
    ...
    const submissions = await gcpClient.getFormSubmissions(FORM_ID, {});
    ...
    // Over Time analysis
    console.log('========================================');
    console.log('OVER TIME ANALYSIS COMPATIBILITY');
    console.log('========================================\n');

    const dateFields = fieldStats.filter(f => getFieldType(fields.find(field => field.id === f.id)) === 'date');
    const valueFields = fieldStats.filter(f => {
      const type = getFieldType(fields.find(field => field.id === f.id));
      return (type === 'number' || type === 'category') && f.filledResponses >= 2;
    });
    ...
    if (dateFields.length === 0) {
      ...
      dateFields.forEach(f => {
        console.log(`   ✅ ${f.label} (${f.filledResponses} responses)`);
      });
      console.log('');
    }

    console.log('📈 VALUE FIELDS (Number/Category):');
    if (valueFields.length === 0) {
      console.log('   ❌ No usable value fields found\n');
    } else {
    ...
    console.log('VALID OVER TIME COMBINATIONS:');
    if (dateFields.length === 0 || valueFields.length === 0) {
      console.log('❌ No valid over time combinations available');
      console.log('   Need: At least 1 date field AND 1 value field (number/category) with ≥2 filled responses\n');
    } else {
      let validCombinations = 0;
      ...
      for (const dateField of dateFields) {
        for (const valueField of valueFields) {
          ...
          for (const submission of submissions) {
            const data = submission[dataFieldName] || {};
            const dateValue = data[dateField.id];
            const value = data[valueField.id];
          ...
          if (commonResponses >= 2) {
            validCombinations++;
            console.log(`✅ ${valueField.label} over ${dateField.label} (${commonResponses} complete pairs)`);
          } else {
            console.log(`❌ ${valueField.label} over ${dateField.label} (only ${commonResponses} complete pairs, need ≥2)`);
          }
        }
      }
    ...
    console.log(`Date fields (usable): ${dateFields.length}`);
    console.log('');
...
// Helper function to determine analytics field type (copied from custom-analytics.js)
